            ...
            <tbody>
              ...
              {products.data?.items.map((p) => (
                <tr key={p.id} className="border-t border-slate-100 dark:border-slate-800">
                  ...
                  <td className="px-3 py-3"><span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-800">{money(p.display_price_per_1000 ?? p.price_per_1000)} {priceUnitSuffix(p)}</span></td>
      ...
      <Card className="lg:col-span-2 h-fit">
        ...
        <form className="mt-4 space-y-4" onSubmit={form.handleSubmit((v) => {
          ...
          <div className="rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-800">
            <p>Price: {money(unit)} {priceUnitSuffix(p)}</p>
            <p>Balance: {me?.wallet ? money(me.wallet.balance) : "Sign in to view"}</p>
            <p className="text-xs text-muted">{isEachPrice(p) ? `${money(unit)} per 1 × ${Number(quantity || 0).toLocaleString()} = ${money(total)}` : `${money(unit)} per 1,000 × ${Number(quantity || 0).toLocaleString()} = ${money(total)}`}</p>
